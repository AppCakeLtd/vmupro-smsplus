...
static const int PSGVolumeValues[2][16] = {
    {892, 892, 892, 760, 623, 497, 404, 323, 257, 198, 159, 123, 96, 75, 60, 0}, /* I can't remember why 892... :P some
                                                                                    scaling I did at some point */
...
void SN76489_SetContext(int which, uint8_t *data) { memcpy(&SN76489[which], data, sizeof(SN76489_Context)); }

void SN76489_GetContext(int which, uint8_t *data) { memcpy(data, &SN76489[which], sizeof(SN76489_Context)); }

uint8_t *SN76489_GetContextPtr(int which) { return (uint8_t *)&SN76489[which]; }

int SN76489_GetContextSize(void) { return sizeof(SN76489_Context); }
...
void SN76489_Write(int which, int data) {
  ...
  if (data & 0x80) {
    ...
    p->Registers[p->LatchedRegister] = (p->Registers[p->LatchedRegister] & 0x3f0) /* zero low 4 bits */
                                       | (data & 0xf);                            /* and replace with data */
  }
  else {
    /* Data byte        %0 - dddddd */
    if (!(p->LatchedRegister % 2) && (p->LatchedRegister < 5))                      /* Tone register */
      p->Registers[p->LatchedRegister] = (p->Registers[p->LatchedRegister] & 0x00f) /* zero high 6 bits */
  ...
  for (j = 0; j < length; j++) {
    for (i = 0; i <= 2; ++i)
      if (p->IntermediatePos[i] != LONG_MIN)
        p->Channels[i] = (p->Mute >> i & 0x1) * PSGVolumeValues[p->VolumeArray][p->Registers[2 * i + 1]] *
                         p->IntermediatePos[i] / 65536;
      else
        p->Channels[i] =
            (p->Mute >> i & 0x1) * PSGVolumeValues[p->VolumeArray][p->Registers[2 * i + 1]] * p->ToneFreqPos[i];

    p->Channels[3] = (short)((p->Mute >> 3 & 0x1) * PSGVolumeValues[p->VolumeArray][p->Registers[7]] *
                             (p->NoiseShiftRegister & 0x1));
    ...
    for (i = 0; i <= 2; ++i) p->ToneFreqVals[i] -= p->NumClocksForSample;
    ...
    for (i = 0; i <= 2; ++i) {
      if (p->ToneFreqVals[i] <= 0) { /* If it gets below 0... */
        if (p->Registers[i * 2] > PSG_CUTOFF) {
          ...
          p->IntermediatePos[i] = (long)((p->NumClocksForSample - p->Clock + 2 * p->ToneFreqVals[i]) *
    ...
    if (p->ToneFreqVals[3] <= 0) {            /* If it gets below 0... */
      ...
      if (p->ToneFreqPos[3] == 1) { /* Only once per cycle... */
        ...
        if (p->Registers[6] & 0x4) { /* White noise */
          ...
          switch (p->WhiteNoiseFeedback) {
            ...
            case 0x0009: /* SMS, GG, MD  %00001001 */
              ...
              /* which one?         Feedback=((p->NoiseShiftRegister&p->WhiteNoiseFeedback) &&
               * (p->NoiseShiftRegister&p->WhiteNoiseFeedback^p->WhiteNoiseFeedback)); */
              Feedback =
                  ((p->NoiseShiftRegister & p->WhiteNoiseFeedback) &&
                   ((p->NoiseShiftRegister & p->WhiteNoiseFeedback) ^ p->WhiteNoiseFeedback));
              break;
        ...
        /* Original code: */
        /*          p->NoiseShiftRegister=(p->NoiseShiftRegister>>1) |
         * ((p->Registers[6]&0x4?((p->NoiseShiftRegister&0x9) &&
         * (p->NoiseShiftRegister&0x9^0x9)):p->NoiseShiftRegister&1)<<15); */
      }
    }
  }
}
