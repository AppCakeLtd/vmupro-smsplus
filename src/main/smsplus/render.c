...
extern const uint8_t *vc_table[2][3];
...
static uint8_t buf_alt = 0;

/* Precalculated pixel table */
static uint16 pixel[PALETTE_SIZE];

// static uint8_t* bg_pattern_cache = ESP32_PSRAM + 0x300000; //[0x20000];/* Cached and flipped patterns */


/* Pixel look-up table */
extern const uint8_t lut[0x10000];
...
static __inline__ uint32 read_dword(void *address) {
  if ((uint32)address & 3) {
    ...
    return (
        *((uint8_t *)address) + (*((uint8_t *)address + 1) << 8) + (*((uint8_t *)address + 2) << 16) +
        (*((uint8_t *)address + 3) << 24)
    );
#else /* big endian version */
    return (
        *((uint8_t *)address + 3) + (*((uint8_t *)address + 2) << 8) + (*((uint8_t *)address + 1) << 16) +
        (*((uint8_t *)address) << 24)
    );
#endif
  }
...
void render_line(int line) {
  ...
  // odd lines to 0, even lines to 1
  buf_alt = (line + 1) % 2 == 0 ? 1 : 0;

  /* Point to current line in output buffer */
  linebuf = &internal_buffer[buf_alt][0];
  ...
  if ((vline < top_border) || (vline >= (bitmap.viewport.h + top_border))) {
    ...
    if ((vdp.mode > 7) && (vdp.reg[1] & 0x40)) render_obj(line);
  ...
  /* Active display */
    ...
    if (vdp.reg[1] & 0x40) {
      ...
      if ((vdp.reg[0] & 0x20) && (IS_SMS || IS_MD)) memset(linebuf, BACKDROP_COLOR, 8);
  ...
  if (view) {
    /* adjust output line */
    if (!overscan) vline -= top_border;

    // if (option.ntsc)
    //   sms_ntsc_blit(&sms_ntsc, ( SMS_NTSC_IN_T const * )pixel, internal_buffer, bitmap.viewport.w +
    //   2*bitmap.viewport.x, vline);
    // else
    remap_8_to_16(vline);
  }
}
...
void render_bg_sms(int line) {
  ...
  uint16 attr;
  uint16 nt_addr =
      (vdp.ntab + ((v_line >> 3) << 6)) & (((sms.console == CONSOLE_SMS) && !(vdp.reg[2] & 1)) ? ~0x400 : 0xFFFF);
  ...
  if (shift) {
    ...
    for (x = shift; x < 8; x++) linebuf[(0 - shift) + (x)] = 0;
  ...
  if (shift) {
    ...
    uint8_t *ptr = (uint8_t *)tile_get(attr, v_row >> 3);
    for (x = 0; x < shift; x++) {
#if 0
...
static void parse_satb(int line) {
  ...
  if (vdp.reg[1] & 0x02) height <<= 1;

  /* Adjust height for zoomed sprites */
  if (vdp.reg[1] & 0x01) height <<= 1;
  ...
  for (i = 0; i < 64; i++) {
    ...
    if (vdp.extended == 0 && yp == 208) return;
...
static inline void remap_8_to_16(int line) {
  ...
  // uint8_t* src = internal_buffer + bitmap.viewport.x;
  //  for(int x = 0; x < bitmap.viewport.w; ++x)
  //  {
  //    *(p++) = pixel[ *(src++) & PIXEL_MASK ];
  //  }

  uint8_t *dst = (uint8_t *)bitmap.data + (line * bitmap.pitch);  // + bitmap.viewport.x;
  // uint8_t* src = internal_buffer + bitmap.viewport.x;
  // for(int x = 0; x < bitmap.viewport.w; ++x)
  // {
  //     *(dst++) = *(src++) & PIXEL_MASK;
...
#else
  // We'll blit two pixels at a time for a whole line and blow it up with its palette as well.
  // Track a global param to see if we're rendering to fill or original size
  // RUNS 2 lines per iteration

  if ((line + 1) % 2 == 0) {
    uint32_t *screen32 = (uint32_t *)bitmap.data;
    uint8_t bufside    = 0;
    int width          = bitmap.vmpro_output.outWidth > 240 ? 240 : bitmap.vmpro_output.outWidth;
    int xstart         = bitmap.vmpro_output.xStart > 0 ? bitmap.vmpro_output.xStart : 0;
    // run the below for this + the previous line
    for (int l = (line - 1); l <= line; ++l) {
      if (bitmap.vmpro_output.fill) {
        int outline = l * 3 / 2;
        for (int p = xstart; p < 240; ++p) {
          int nearcol     = (p * 2) / 3;
          uint16_t pixel1 = pixel[internal_buffer[bufside][nearcol + bitmap.vmpro_output.fboffset] % PALETTE_SIZE];
          uint16_t pixel2 = pixel[internal_buffer[bufside][nearcol + bitmap.vmpro_output.fboffset + 1] % PALETTE_SIZE];

          uint32_t combinedPixels                                                        = (pixel2 << 16) | pixel1;
          screen32[(((outline + bitmap.vmpro_output.yOffset) * 240) + (p - xstart)) / 2] = combinedPixels;

          if (bufside) {
            screen32[(((outline + bitmap.vmpro_output.yOffset + 1) * 240) + (p - xstart)) / 2] =
                combinedPixels;  // copy
          }
        }
      }
        ...
        for (int p = xstart; p <= width + xstart; p += 2) {
          uint16_t pixel1 = pixel[internal_buffer[bufside][p + bitmap.vmpro_output.fboffset] % PALETTE_SIZE];
          uint16_t pixel2 = pixel[internal_buffer[bufside][p + bitmap.vmpro_output.fboffset + 1] % PALETTE_SIZE];

          uint32_t combinedPixels                                                  = (pixel2 << 16) | pixel1;
          screen32[(((l + bitmap.vmpro_output.yOffset) * 240) + (p - xstart)) / 2] = combinedPixels;
        }
      }
...
void render_copy_palette(uint16 *palette) { memcpy(palette, pixel, sizeof(pixel)); }
