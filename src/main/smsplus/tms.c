...
extern const uint8_t tms_lookup[16][256][2]; /* Expand BD, PG data into 8-bit pixels (G1,G2) */
extern const uint8_t mc_lookup[16][256][8];  /* Expand BD, PG data into 8-bit pixels (MC) */
extern const uint8_t txt_lookup[256][2];     /* Expand BD, PG data into 8-bit pixels (TX) */
extern const uint8_t bp_expand[256][8];      /* Expand PG data into 8-bit pixels */
extern const uint8_t tms_obj_lut[16 * 256];  /* Look up priority between SG and display pixels */

static uint8_t diff_mask[]  = {0x07, 0x07, 0x0F, 0x0F};
static uint8_t name_mask[]  = {0xFF, 0xFF, 0xFC, 0xFC};
static uint8_t diff_shift[] = {0, 1, 0, 1};
static uint8_t size_tab[]   = {8, 16, 16, 32};
...
static void render_bg_m1(int line) {
  ...
  /* V3 */
  if ((vdp.line & 7) == 7) text_counter += 40;
