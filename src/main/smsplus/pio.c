...
/*
  ...
  This port is used to control the two input/output lines available on each controller port (referred to as ports A and
  B). Some control hardware needs to recieve input from the CPU to perform its task, and uses one or more of these
  lines. Passive hardware uses all of them as inputs.
...
void pio_shutdown(void) { /* Nothing to do */ }
...
void pio_ctrl_w(uint8_t data) {
  ...
  if ((io_current->th_dir[0] == PIN_DIR_IN) && (io_current->th_level[0] == PIN_LVL_HI) && (th_level[0] == PIN_LVL_LO)) {
...
static uint8_t device_r(int port) {
  uint8_t temp = 0x7F;
...
uint8_t coleco_pio_r(int port) {
  ...
  if (coleco.pio_mode) {
    ...
    /* KeyPad (0-9,*,#) */
    if (coleco.keypad[port] < 12) temp = keymask[coleco.keypad[port]];
